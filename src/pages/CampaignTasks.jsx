...
const sampleData = [
  { id: 1, slNo: 1, title: 'Email Campaign - Q1 Promo', category: 'Email', deal: 'Website Development', dealId: 'DL001', amount: 150000, description: 'Send promotional emails', scheduledDate: '2024-01-20', scheduledTime: '10:00', assignedBy: 'Admin', assignedTo: 'John Doe', priority: 'high', status: 'pending' },
  { id: 2, slNo: 2, title: 'SMS Campaign - New Launch', category: 'SMS', deal: 'CRM Implementation', dealId: 'DL002', amount: 200000, description: 'Launch SMS campaign', scheduledDate: '2024-01-18', scheduledTime: '14:00', assignedBy: 'Admin', assignedTo: 'Jane Smith', priority: 'medium', status: 'completed' },
  { id: 3, slNo: 3, title: 'WhatsApp Broadcast', category: 'WhatsApp', deal: 'Annual Maintenance', dealId: 'DL003', amount: 50000, description: 'Broadcast message', scheduledDate: '2024-01-15', scheduledTime: '11:00', assignedBy: 'Admin', assignedTo: 'Mike Johnson', priority: 'low', status: 'overdue' },
  { id: 4, slNo: 4, title: 'Social Media Campaign', category: 'Social', deal: 'Website Development', dealId: 'DL001', amount: 150000, description: 'Social media posts', scheduledDate: '2024-01-25', scheduledTime: '15:00', assignedBy: 'Admin', assignedTo: 'John Doe', priority: 'high', status: 'pending' },
];
...
const CampaignTasks = () => {
  ...
  const [filters, setFilters] = useState({ deal: '', status: '', assignedBy: '', assignedTo: '', category: '', dateRange: { start: '', end: '' } });

  const columns = [
    { key: 'checkbox', label: '' }, { key: 'slNo', label: 'Sl No' }, { key: 'title', label: 'Title', sortable: true },
    { key: 'category', label: 'Category', sortable: true }, { key: 'deal', label: 'Deal', sortable: true },
    { key: 'amount', label: 'Amount', sortable: true }, { key: 'description', label: 'Description' },
    { key: 'scheduledDate', label: 'Scheduled Date', sortable: true }, { key: 'assignedBy', label: 'Assigned By', sortable: true },
    { key: 'assignedTo', label: 'Assigned To', sortable: true }, { key: 'status', label: 'Status', sortable: true }
  ];

  const filteredData = useMemo(() => {
    let data = [...tasks];
    if (searchQuery) data = data.filter(item => item.title.toLowerCase().includes(searchQuery.toLowerCase()) || item.deal.toLowerCase().includes(searchQuery.toLowerCase()) || item.category.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleExportCSV = () => {
    ...
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'campaign-tasks.csv'; link.click();
  };
  const getStatusBadge = (status) => { const statusMap = { 'completed': 'status-completed', 'pending': 'status-pending', 'overdue': 'status-overdue' }; const labelMap = { 'completed': 'Completed', 'pending': 'Pending', 'overdue': 'OverDue' }; return <span className={`badge badge-${statusMap[status]}`}>{labelMap[status]}</span>; };
  ...
  return (
    <div className="enquiries-page">
      <PageHeader title="Campaign Tasks" description="Manage campaign-related tasks and activities." />
      {renderStatsCards()}
      <div className="task-actions-row">
        <div className="task-actions-left"><button className="btn btn-assign"><User size={16} />Assign To</button><button className="btn btn-secondary" onClick={() => setShowFilters(!showFilters)}><Filter size={16} />Filter</button></div>
        <div className="task-actions-right"><button className="btn btn-secondary" onClick={handleExportCSV}><Download size={16} />Export</button><button className="btn btn-primary"><Plus size={16} />Campaign Task</button></div>
      </div>
      {showFilters && (
        <div className="filter-actions-panel">
          <div className="filter-row">
            <div className="filter-group"><label>Deal</label><select value={filters.deal} onChange={(e) => setFilters({ ...filters, deal: e.target.value })}><option value="">All</option><option value="Website Development">Website Development</option><option value="CRM Implementation">CRM Implementation</option></select></div>
            ...
            <div className="filter-group"><label>Assigned To</label><select value={filters.assignedTo} onChange={(e) => setFilters({ ...filters, assignedTo: e.target.value })}><option value="">All</option><option value="John Doe">John Doe</option><option value="Jane Smith">Jane Smith</option></select></div>
            <div className="filter-group"><label>Category</label><select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })}><option value="">All</option><option value="Email">Email</option><option value="SMS">SMS</option><option value="WhatsApp">WhatsApp</option><option value="Social">Social</option></select></div>
          </div>
          <div className="filter-row"><div className="filter-group"><label>Date Range</label><div className="date-range-input"><input type="date" value={filters.dateRange.start} onChange={(e) => setFilters({ ...filters, dateRange: { ...filters.dateRange, start: e.target.value } })} /><span>to</span><input type="date" value={filters.dateRange.end} onChange={(e) => setFilters({ ...filters, dateRange: { ...filters.dateRange, end: e.target.value } })} /></div></div><div className="filter-actions-btns"><button className="btn btn-primary" onClick={() => setShowFilters(false)}>Apply Filter</button><button className="btn btn-secondary" onClick={clearFilters}>Reset</button></div></div>
        </div>
      )}
      <div className="enquiries-toolbar">
        <div className="toolbar-left"><div className="search-box"><Search size={16} className="search-icon" /><input type="text" placeholder="Search tasks..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="search-input" /></div>
        <div className="dropdown-container"><button className="btn btn-secondary" onClick={() => setShowSortDropdown(!showSortDropdown)}>Sort By<ChevronDown size={14} /></button>{showSortDropdown && (<div className="sort-dropdown"><button onClick={() => { handleSort('scheduledDate'); setShowSortDropdown(false); }}>Scheduled Date</button><button onClick={() => { handleSort('title'); setShowSortDropdown(false); }}>Title</button><button onClick={() => { handleSort('status'); setShowSortDropdown(false); }}>Status</button></div>)}</div>
        </div>
      ...
      <div className="pagination-container">
        ...
        <div className="pagination-right"><button className="pagination-btn" disabled={currentPage === 1} onClick={() => setCurrentPage(1)}>First</button><button className="pagination-btn" disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)}><ChevronLeft size={16} /></button><span className="page-indicator">Page {currentPage} of {totalPages}</span><button className="pagination-btn" disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)}><ChevronRight size={16} /></button><button className="pagination-btn" disabled={currentPage === totalPages} onClick={() => setCurrentPage(totalPages)}>Last</button></div>
      </div>
    </div>
  );
};
