 ...
 */
const Drawer = ({ isOpen, onClose, children, title, overlayClassName, panelClassName, animated = false, exitDuration = 300, closeOnEsc = false, closeOnBackdropClick = true, }) => {
    ...
    if (!shouldRender)
        return null;
    return (_jsx("div", { className: `${overlayClassName}${animated && isOpen ? ' visible' : ''}`, onClick: closeOnBackdropClick ? onClose : undefined, children: _jsxs("div", { className: `${panelClassName}${animated && isAnimating ? ' animating' : ''}`, onClick: (e) => e.stopPropagation(), children: [title !== undefined && (_jsxs("div", { className: "drawer-header", children: [_jsx("h2", { children: title }), _jsx("button", { className: "drawer-close", onClick: onClose, children: _jsx(X, { size: 20 }) })] })), children] }) }));
};
