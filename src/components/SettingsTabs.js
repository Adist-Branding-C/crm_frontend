...
const SettingsTabs = ({ items = tabsConfig }) => {
    ...
    return (_jsxs("div", { className: "settings-tabs", children: [items.slice(0, limit).map((item) => (_jsxs(Link, { to: item.link, className: `settings-tab ${location.pathname === item.link ? 'active' : ''}`, children: [_jsx(item.icon, { size: 16 }), _jsx("span", { style: { paddingLeft: "10px" }, children: item.title })] }, item.id))), items.length > 8 && (_jsxs(Link
            //When clicking this button i need to show reset are links here
            , { 
                //When clicking this button i need to show reset are links here
                onClick: () => setLimit(limit > 8 ? 8 : items.length), className: `settings-tab ${location.pathname === items[8].link ? 'active' : ''}`, children: [limit > 8 ? _jsx(MinusIcon, {}) : _jsx(PlusIcon, {}), _jsx("span", { style: { paddingLeft: "10px" }, children: limit > 8 ? "Reset" : "More" })] }))] }));
};
