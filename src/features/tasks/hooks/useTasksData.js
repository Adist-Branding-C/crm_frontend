...
export const useTasksData = () => {
    ...
    const handleMarkCompleted = (id) => {
        setTasks(prev => prev.map(task => task.id === id ? { ...task, status: TaskStatus.COMPLETED } : task));
        setActionMenuOpen(null);
    };
    const handleExportCSV = () => {
        ...
        const csvContent = [
            headers.join(','),
            ...filteredData.map(task => [task.slNo, `"${task.title}"`, task.category, `"${task.deal}"`, task.amount, `"${task.description}"`, task.scheduledDate, task.assignedBy, task.assignedTo, task.status].join(',')),
        ].join('\n');
...
//# sourceMappingURL=useTasksData.js.map