...
const TasksPage = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (sortConfig.key) data.sort((a, b) => { const aVal = a[sortConfig.key as keyof TaskItem]; const bVal = b[sortConfig.key as keyof TaskItem]; if (aVal < bVal) return sortConfig.direction === 'asc' ? -1 : 1; if (aVal > bVal) return sortConfig.direction === 'asc' ? 1 : -1; return 0; });
    return data;
  ...
  const handleDeleteTask = (id: number) => { setTasks(prev => prev.filter(task => task.id !== id)); setActionMenuOpen(null); };
  const handleMarkCompleted = (id: number) => { setTasks(prev => prev.map(task => task.id === id ? { ...task, status: 'completed' as const } : task)); setActionMenuOpen(null); };
  ...
  const handleExportCSV = () => {
    ...
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'tasks.csv'; link.click();
  };

  const getStatusBadge = (status: string) => {
    const statusMap: Record<string, string> = { 'completed': 'status-completed', 'pending': 'status-pending', 'overdue': 'status-overdue' };
    const labelMap: Record<string, string> = { 'completed': 'Completed', 'pending': 'Pending', 'overdue': 'OverDue' };
      ...
      <div className="account-content" style={{ width: '100%', maxWidth: '100%' }}>
        ...
        <div className="task-actions-row">
          ...
          <div className="task-actions-right">
            ...
            <button className="btn btn-primary" onClick={() => setIsDrawerOpen(true)}><Plus size={16} />Task</button>
          </div>
        </div>
        ...
        {showFilters && (
          <div className="filter-actions-panel">
            <div className="filter-row">
              ...
              <div className="filter-group"><label>Category</label><select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })}><option value="">All</option><option value="Follow Up">Follow Up</option><option value="Demo">Demo</option><option value="Payment Reminder">Payment Reminder</option></select></div>
            </div>
            <div className="filter-row">
                    ...
                    <div className="action-menu-container">
                      ...
                      {actionMenuOpen === row.id && (
                        <div className="action-dropdown">
                          <button><Eye size={14} />View Task</button>
                          <button><Edit2 size={14} />Edit Task</button>
                          <button onClick={() => handleMarkCompleted(row.id)}><CheckCircle size={14} />Mark Completed</button>
                          <button><User size={14} />Reassign</button>
                          <button onClick={() => handleDeleteTask(row.id)} className="delete"><Trash2 size={14} />Delete</button>
                        </div>
                      )}
                    </div>
                  </td>
        ...
        <div className="pagination-container">
          <div className="pagination-left">
            <span className="pagination-info">Showing {startIndex + 1}-{Math.min(startIndex + rowsPerPage, filteredData.length)} of {filteredData.length} entries</span>
          </div>
        ...
        <AddDealTaskDrawer isOpen={isDrawerOpen} onClose={() => { setIsDrawerOpen(false); setEditingTask(null); }} task={editingTask as any} onSave={() => { }} />
      </div>
    </div>
  );
};
