...
import type { DealItem, DealFilters } from '../types';
...
export function useDealsData() {
  ...
  const { filteredData: searchedData, setQuery: setSearchQuery, query: searchQuery } = useSearchFilter(
    deals,
    ['dealName', 'lead', 'mobile', 'dealId' as keyof DealItem]
  );
  ...
  const totalDealAmount = useMemo(
    () => filteredData.reduce((sum, deal) => sum + (Number(deal.amount) || 0), 0),
    [filteredData]
  );
  ...
  const handleEditDeal = useCallback((deal: DealItem) => {
    setEditingDeal(deal);
    setIsDrawerOpen(true);
  ...
  const handleSaveDeal = useCallback((data: {
    dealName: string; lead: string; mobile: string; amount: string;
    status: string; type: string; startDate: string; endDate: string; assignAgent: string;
  }) => {
    if (editingDeal) {
