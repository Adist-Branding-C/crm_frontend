...
const initialData: CallReasonItem[] = [];
...
const CallReasonPage = () => {
  ...
  const handleEditClick = (item: CallReasonItem) => {
    setShowForm(true);
    setEditingItem(item);
  ...
  const handleDeleteClick = (item: CallReasonItem) => {
          ...
          {menuItems.map(item => {
            ...
            return (
              <Link
                key={item.id}
                to={item.link}
                className={`menu-item ${item.id === 'reason' ? 'active' : ''}`}
              >
                <Icon size={18} />
                {item.label}
              </Link>
        ...
        <div className="drawer-overlay" onClick={handleCloseForm}>
          <div className="drawer drawer-right" onClick={(e) => e.stopPropagation()}>
            <div className="drawer-header">
              <h5>{editingItem ? 'Edit Reason' : 'Add Reason'}</h5>
