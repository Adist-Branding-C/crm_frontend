...
const CallStatusPage = () => {
    ...
    const handleEditSubmit = useCallback(async (values, helpers) => {
        ...
        try {
            const { name, status } = values;
            const response = await update.update(drawer.editingItem.id, { name: name.trim(), status });
            if (!response) {
                fetch.setError('Network error. Please try again.');
                return false;
            }
    ...
    return (_jsxs("div", { className: "task-settings-page", children: [_jsx(PageHeader, { title: "Task Settings", description: "Manage task configurations and settings" }), _jsx(SettingsTabs, { tabs: SETTINGS_TABS }), _jsxs("div", { className: "account-content", children: [_jsxs("div", { className: "call-status-table-wrapper", children: [_jsx(TableToolbar, { searchQuery: searchValue, onSearchChange: handleSearchInput, rowsPerPage: fetch.limit, onRowsPerPageChange: fetch.handleRowsPerPageChange, onAdd: drawer.openAddDrawer, addLabel: "Add Call Status" }), _jsx("div", { className: "table-scroll", children: _jsxs(Table, { className: "data-table", children: [_jsx(THead, { children: _jsxs(TRow, { children: [_jsx(TCell, { variant: "th", children: "Sl No" }), _jsx(TCell, { variant: "th", children: "Name" }), _jsx(TCell, { variant: "th", children: "Status" }), _jsx(TCell, { variant: "th", children: "Actions" })] }) }), _jsx(TBody, { children: fetch.callStatusList.length === 0 ? (_jsx(TRow, { children: _jsx(TCell, { colSpan: 4, className: "dataTables_empty", children: "No data available in table" }) })) : (fetch.callStatusList.map((item, index) => (_jsxs(TRow, { children: [_jsx(TCell, { children: startIndex + index + 1 }), _jsx(TCell, { children: item.name || '-' }), _jsx(TCell, { children: _jsx(SettingsStatusBadge, { status: item.status }) }), _jsx(TCell, { children: _jsx(CallStatusActions, { item: item, dropdownOpen: table.dropdownOpen, onToggleDropdown: table.toggleDropdown, onEdit: drawer.openEditDrawer, onDelete: table.handleDeleteClick }) })] }, item.id)))) })] }) }), _jsx(TablePagination, { currentPage: fetch.pageNumber, totalPages: totalPages, rowsPerPage: fetch.limit, totalRecords: fetch.totalCount, onPageChange: fetch.setPageNumber })] }), _jsx(AddCallStatusDrawer, { isOpen: drawer.showAddDrawer, onClose: drawer.closeAddDrawer, validationSchema: addCallStatusValidationSchema, initialValues: ADD_CALL_STATUS_INITIAL_VALUES, onSubmit: handleAddSubmit, isLoading: fetch.isLoading, error: fetch.error }), _jsx(EditCallStatusDrawer, { isOpen: drawer.showEditDrawer, onClose: drawer.closeEditDrawer, validationSchema: editCallStatusValidationSchema, initialValues: form.editInitialValues, onSubmit: handleEditSubmit, isLoading: fetch.isLoading, error: fetch.error, editingItem: drawer.editingItem }), _jsx(DeleteCallStatusDialog, { isOpen: !!table.deletingItem, itemName: table.deletingItem?.name || '', onConfirm: handleConfirmDelete, onClose: table.closeDeleteDialog })] }), _jsx(ToastNotification, { message: toastMessage, type: toastType, visible: showToast, onClose: () => setShowToast(false) })] }));
};
