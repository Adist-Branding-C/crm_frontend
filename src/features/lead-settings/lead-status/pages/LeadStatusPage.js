...
import { EMPTY_LEAD_STATUS_FORM_DATA, ADD_LEAD_STATUS_LABEL, LEAD_STATUS_COLUMN_ADDED_BY, LEAD_STATUS_COLUMN_STATUS, LEAD_STATUS_COLUMN_COLOR, LEAD_STATUS_COLUMN_CONVERSION, } from '../constants';
...
const LeadStatusPage = () => {
    ...
    return (_jsxs("div", { className: "lead-settings-page", children: [_jsx(PageHeader, { title: "Lead Status", description: "Manage lead statuses and conversion metrics" }), _jsx(LeadSettingsSidebar, {}), _jsx("div", { className: "settings-content", children: _jsxs("div", { className: "table-container", children: [_jsxs(TableNav, { searchQuery: search.searchValue, onSearchChange: search.handleSearchChange, rowsPerPage: table.limit, onRowsPerPageChange: actions.handleRowsPerPageChange, children: [_jsx(SortToggleButton, { sortOrder: table.sortOrder, onToggle: table.toggleSortOrder }), _jsxs("button", { className: "btn btn-primary", onClick: drawer.openAddDrawer, children: [_jsx(Plus, { size: 16 }), " ", ADD_LEAD_STATUS_LABEL] })] }), _jsxs(Table, { wrapperClassName: "table-scroll", className: "data-table", children: [_jsx(THead, { children: _jsxs(TRow, { children: [_jsx(TCell, { variant: "th", children: LABEL_SL_NO }), _jsx(TCell, { variant: "th", children: LEAD_STATUS_COLUMN_ADDED_BY }), _jsx(TCell, { variant: "th", children: LEAD_STATUS_COLUMN_STATUS }), _jsx(TCell, { variant: "th", children: LEAD_STATUS_COLUMN_COLOR }), _jsx(TCell, { variant: "th", children: LEAD_STATUS_COLUMN_CONVERSION }), _jsx(TCell, { variant: "th", children: LABEL_ACTIONS })] }) }), _jsx(TBody, { children: table.list.length === 0 ? (_jsx(EmptyState, { colSpan: 6, message: LABEL_NO_DATA })) : (table.list.map((item, idx) => (_jsx(LeadStatusRow, { item: item, index: table.startIndex + idx, isMenuOpen: dropdown.dropdownOpen === item.id, onToggleMenu: (open) => dropdown.toggleDropdown(open ? item.id : null), onEdit: actions.handleEditClick, onDelete: actions.handleDeleteClick }, item.id)))) })] }), _jsx(AdminPagination, { currentPage: table.pageNumber, totalPages: table.totalPages, startIndex: table.startIndex, rowsPerPage: table.limit, totalItems: table.totalCount, onPageChange: table.setPageNumber, onRowsPerPageChange: actions.handleRowsPerPageChange, prevNextOnly: true, alwaysShowNav: true })] }) }), _jsx(DrawerShell, { isOpen: drawer.showDrawer, title: drawer.editingItem ? `${ACTION_EDIT} Lead Status` : `${ACTION_ADD} Lead Status`, onClose: drawer.closeDrawer, children: _jsx(LeadStatusForm, { form: {
                    ...
                    } }) }), _jsx(AdminDeleteModal, { isOpen: !!deleteConfirm.deletingItem, itemName: deleteConfirm.deletingItem?.status, itemType: "status", error: table.error, onConfirm: deleteConfirm.handleConfirmDelete, onClose: deleteConfirm.closeDeleteModal })] }));
};
export default LeadStatusPage;
//# sourceMappingURL=LeadStatusPage.js.map