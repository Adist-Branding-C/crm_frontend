...
import type { LeadSourceFormProps } from '../types/lead-source-form.types';

const LeadSourceForm = ({ form, status }: LeadSourceFormProps) => {
          ...
          <div ref={formBodyRef}>
            <Form>
              ...
              <div className="form-group">
                <label>{LEAD_SOURCE_FIELD_LABEL} <span className="text-danger">*</span></label>
                <Field type="text" name="source" className={fieldClass('source')} placeholder={LEAD_SOURCE_FIELD_PLACEHOLDER} />
                {showError('source') && errors.source && <small className="field-error-text">{errors.source}</small>}
              </div>
