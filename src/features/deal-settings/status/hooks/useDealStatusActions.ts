...
import type { UseDealStatusActionsParams } from '../types/use-deal-status-actions.types';
import type { DealStatusItem } from '../types/deal-status.types';

export function useDealStatusActions({
  feature, drawer, refetch,
}: UseDealStatusActionsParams) {
  const [deletingItem, setDeletingItem] = useState<DealStatusItem | null>(null);

  const handleSave = useCallback(async () => {
    if (drawer.editingItem) {
      const response = await dealStatusService.updateDealStatus(drawer.editingItem.id, drawer.formData);
      if (response.status) {
        feature.setDealStatusList(prev => prev.map(item =>
          item.id === drawer.editingItem!.id ? { ...item, name: drawer.formData.name, status: drawer.formData.status } : item
        ));
        drawer.closeDrawer();
      } else {
  ...
  const handleDeleteClick = useCallback((item: DealStatusItem) => {
