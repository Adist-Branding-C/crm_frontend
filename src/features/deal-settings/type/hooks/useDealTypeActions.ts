...
import type { UseDealTypeActionsParams } from '../types/use-deal-type-actions.types';
import type { DealTypeItem } from '../types/deal-type.types';

export function useDealTypeActions({
  feature, drawer, refetch,
}: UseDealTypeActionsParams) {
  ...
  const handleSave = useCallback(async () => {
    if (drawer.editingItem) {
      ...
      if (response.status) {
        feature.setDealTypeList(prev => prev.map(item =>
  ...
  const handleDeleteClick = useCallback((item: DealTypeItem) => {
