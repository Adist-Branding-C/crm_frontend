...
const SetupPage = () => {
  ...
  return (
    <PageContainer>
      <PageHeader title="Setup" description="Configure your CRM pipeline and tracking stages" />
      ...
      <div className="setup-content">
        <div className="setup-card">
            ...
            {stages.map((stage, index) => (
              <React.Fragment key={stage.id}>
                <div className="stage-item">
                  <div className="stage-drag"><GripVertical size={16} /></div>
                  <div className="stage-color" style={{ background: stage.color }} />
                  {editingStage === stage.id ? (
                    <div className="stage-edit-form">
                      <input type="text" value={stage.name}
                        onChange={(e) => handleUpdateStage(stage.id, e.target.value, stage.color)} autoFocus />
                      <input type="color" value={stage.color}
                        onChange={(e) => handleUpdateStage(stage.id, stage.name, e.target.value)} />
                      <button onClick={() => setEditingStage(null)}><Check size={16} /></button>
                    </div>
                  ) : (
                    <span className="stage-name" onClick={() => setEditingStage(stage.id)}>{stage.name}</span>
                  )}
                  <div className="stage-actions">
                    <button onClick={() => moveStage(index, -1)} disabled={index === 0}>↑</button>
                    <button onClick={() => moveStage(index, 1)} disabled={index === stages.length - 1}>↓</button>
                    <button onClick={() => handleDeleteStage(stage.id)} className="delete-btn"><Trash2 size={14} /></button>
                  </div>
                </div>
                {index < stages.length - 1 && (
                  <div className="stage-connector"><ArrowRight size={16} /></div>
                )}
              </React.Fragment>
            ))}
          </div>
          ...
          <div className="add-stage-form">
            <input type="text" placeholder="New stage name" value={newStageName}
              onChange={(e) => setNewStageName(e.target.value)} />
            <button onClick={handleAddStage}><Plus size={16} /> Add Stage</button>
          </div>

          <div className="form-actions">
            <button className="btn btn-secondary" onClick={handleReset}>Reset Changes</button>
