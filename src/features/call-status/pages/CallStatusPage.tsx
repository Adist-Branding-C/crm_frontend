...
const initialData: CallStatusItem[] = [
  { id: 1, name: 'Connected' },
...
const CallStatusPage = () => {
  ...
  const handleEditClick = (item: CallStatusItem) => {
    setShowForm(true);
    setEditingItem(item);
  ...
  const handleDeleteClick = (item: CallStatusItem) => {
          ...
          {menuItems.map(item => {
            ...
            return (
              <Link
                key={item.id}
                to={item.link}
                className={`menu-item ${item.id === 'call_status' ? 'active' : ''}`}
              >
                <Icon size={18} />
                {item.label}
              </Link>
        ...
        <div className="drawer-overlay" onClick={handleCloseForm}>
          <div className="drawer drawer-right" onClick={(e) => e.stopPropagation()}>
            <div className="drawer-header">
              <h5>{editingItem ? 'Edit Call Status' : 'Add Call Status'}</h5>
