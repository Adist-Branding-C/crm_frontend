...
import type { CampaignTaskItem, CampaignTaskFormData, FetchHandlers, ToastHandlers, SubmitHandlerConfig } from '../types/index';

export function useCampaignTaskSubmitHandlers(
  config: SubmitHandlerConfig<CampaignTaskItem>,
  fetch: FetchHandlers,
  toast: ToastHandlers,
) {
  ...
  const handleAddSubmit = useCallback(async (
    ...
    { setFieldError, setSubmitting }: FormikHelpers<CampaignTaskFormData>,
  ) => {
    fetch.setError('');
  ...
  const handleEditSubmit = useCallback(async (
    ...
    const original: CampaignTaskFormData = {
      title: item.title || '',
