...
import { useCreateDealTask } from './useCreateDealTask';
import { useUpdateDealTask } from './useUpdateDealTask';
import { useDeleteDealTask } from './useDeleteDealTask';
import { useDrawerScroll } from '../../hooks/useDrawerScroll';
import { applyFieldErrors } from '../../shared/utils/applyFieldErrors';
import type { DealTaskItem, DealTaskFormData, FetchHandlers, ToastHandlers, SubmitHandlerConfig } from '../types/index';

export function useDealTaskSubmitHandlers(
  config: SubmitHandlerConfig<DealTaskItem>,
  fetch: FetchHandlers,
  toast: ToastHandlers,
) {
  const create = useCreateDealTask();
  const update = useUpdateDealTask();
  const removal = useDeleteDealTask();
  const { scrollAndFocusError, scrollToTop } = useDrawerScroll();

  const handleAddSubmit = useCallback(async (
    values: DealTaskFormData,
    { setFieldError, setSubmitting }: FormikHelpers<DealTaskFormData>,
  ) => {
    fetch.setError('');
    ...
    try {
      ...
      if (response.status) {
        fetch.refresh();
        toast.showToastMessage('Deal task added successfully', 'success');
        scrollToTop();
        config.onAddSuccess();
        return true;
      }

      const msg = response.message || 'Failed to add deal task';
      ...
      const msg = axiosErr.response?.data?.message || 'Failed to add deal task';
  ...
  const handleEditSubmit = useCallback(async (
    ...
    const original: DealTaskFormData = {
      title: item.title || '',
    ...
    try {
      ...
      if (response.status) {
        fetch.refresh();
        toast.showToastMessage('Deal task updated successfully', 'success');
        scrollToTop();
        config.onEditSuccess();
        return true;
      }

      const msg = response.message || 'Failed to update deal task';
      const mappedField = applyFieldErrors(response.errors, response.field ? (response.errors?.[response.field]?.[0] || msg) : msg, response.field, setFieldError);
      if (!mappedField) {
        fetch.setError(msg);
      }
      ...
      const axiosErr = err as { response?: { data?: { message?: string; errors?: Record<string, string[]>; field?: string } } };
      const msg = axiosErr.response?.data?.message || 'Failed to update deal task';
      const mappedField = applyFieldErrors(axiosErr.response?.data?.errors, msg, axiosErr.response?.data?.field, setFieldError);
      if (!mappedField) {
        fetch.setError(msg);
      }
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      if (response?.status) {
        fetch.refresh();
        toast.showToastMessage('Deal task deleted successfully', 'success');
        config.onDeleteSuccess();
      } else {
        toast.showToastMessage(response?.message || 'Failed to delete deal task', 'error');
      }
    } catch {
      toast.showToastMessage('Failed to delete deal task', 'error');
    }
