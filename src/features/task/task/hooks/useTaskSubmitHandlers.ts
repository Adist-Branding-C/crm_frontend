...
import type { TaskItem, TaskFormData, FetchHandlers, ToastHandlers, SubmitHandlerConfig } from '../types/index';

export function useTaskSubmitHandlers(
  config: SubmitHandlerConfig<TaskItem>,
  fetch: FetchHandlers,
  toast: ToastHandlers,
) {
  ...
  const handleAddSubmit = useCallback(async (
    values: TaskFormData,
    { setFieldError, setSubmitting }: FormikHelpers<TaskFormData>,
  ) => {
    fetch.setError('');
    ...
    try {
      ...
      if (response.status) {
        fetch.refresh();
        toast.showToastMessage('Task added successfully', 'success');
        scrollToTop();
        config.onAddSuccess();
        return true;
      }

      const msg = response.message || 'Failed to add task';
      ...
      const msg = axiosErr.response?.data?.message || 'Failed to add task';
  ...
  const handleEditSubmit = useCallback(async (
    ...
    try {
      ...
      const msg = response.message || 'Failed to update task';
      const mappedField = applyFieldErrors(response.errors, response.field ? (response.errors?.[response.field]?.[0] || msg) : msg, response.field, setFieldError);
      if (!mappedField) {
        fetch.setError(msg);
      }
      ...
      const axiosErr = err as { response?: { data?: { message?: string; errors?: Record<string, string[]>; field?: string } } };
      const msg = axiosErr.response?.data?.message || 'Failed to update task';
      const mappedField = applyFieldErrors(axiosErr.response?.data?.errors, msg, axiosErr.response?.data?.field, setFieldError);
      if (!mappedField) {
        fetch.setError(msg);
      }
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      if (response?.status) {
        fetch.refresh();
        toast.showToastMessage('Task deleted successfully', 'success');
        config.onDeleteSuccess();
      } else {
        toast.showToastMessage(response?.message || 'Failed to delete task', 'error');
      }
    } catch {
      toast.showToastMessage('Failed to delete task', 'error');
    }
