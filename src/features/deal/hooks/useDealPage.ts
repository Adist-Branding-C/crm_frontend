...
export function useDealPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (dealFilters.status) data = data.filter(item => item.status === dealFilters.status);
    if (dealFilters.type) data = data.filter(item => item.type === dealFilters.type);
    if (dealFilters.assignedTo) data = data.filter(item => item.agent === dealFilters.assignedTo);
    return data;
  ...
  const totalDealAmount = useMemo(
    () => filteredData.reduce((sum, d) => sum + (Number(d.amount) || 0), 0),
    [filteredData]
  );
  ...
  const handleEditDeal = useCallback((item: DealItem) => {
    drawer.openEditDrawer(item);
    setActionMenuOpen(null);
  }, [drawer]);

  const handleDeleteDeal = useCallback((id: number) => {
  ...
  const handleDrawerSave = useCallback((data: any) => {
    const helpers = { setSubmitting: () => {}, resetForm: () => {} } as unknown as FormikHelpers<DealFormData>;
    if (drawer.editingItem) {
      actions.handleEditSubmit({ ...data, assignedTo: data.assignAgent } as DealFormData, helpers);
    } else {
      actions.handleSubmit({ ...data, assignedTo: data.assignAgent } as DealFormData, helpers);
    }
  ...
  const handleEditClick = useCallback((item: DealItem) => {
    drawer.openEditDrawer(item);
