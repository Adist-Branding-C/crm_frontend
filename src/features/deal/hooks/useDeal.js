...
export function useDeal() {
    ...
    const fetchDeals = useCallback(async () => {
        ...
        try {
            ...
            if (response.status) {
                ...
                setDealList((Array.isArray(items) ? items : []).map((item) => ({
                    ...item,
    ...
    const handleAddDeal = useCallback(async (values, { setSubmitting, resetForm }) => {
        setError('');
        setIsLoading(true);
        try {
            ...
            if (response.status) {
                ...
                if (newItemId) {
                    setDealList(prev => [...prev, { id: newItemId, ...values }]);
                }
                else {
                    fetchDeals();
                }
    ...
    const handleUpdateDeal = useCallback(async (dealId, values, { setSubmitting }) => {
        setError('');
        setIsLoading(true);
        try {
            const response = await dealService.updateDeal(dealId, values);
            if (response.status) {
                setDealList(prev => prev.map(item => item.dealId === dealId ? { ...item, ...values } : item));
                return true;
            }
            else {
...
//# sourceMappingURL=useDeal.js.map