...
const DepartmentTable = ({ data, searchQuery, onSearchChange, rowsPerPage, onRowsPerPageChange, totalRecords, pageNumber, onPageChange, dropdownOpen, onToggleDropdown, onEdit, onDelete }: DepartmentTableProps) => {
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((item, index) => (
                <tr key={item.id}>
                  <td>{index + 1}</td>
                  <td>{item.departmentName || item.name || '-'}</td>
