...
export function useCheckoutNote() {
    ...
    const handleAddCheckoutNote = useCallback(async (values, { setSubmitting, resetForm, setFieldError }) => {
        pagination.setError('');
    ...
    const handleUpdateCheckoutNote = useCallback(async (id, values, { setSubmitting, setFieldError }) => {
    ...
    const handleDeleteCheckoutNote = useCallback(async (id) => {
        const noteId = Number(id);
        if (!noteId || isNaN(noteId)) {
            pagination.setError('Invalid checkout note id');
            return false;
...
//# sourceMappingURL=useCheckoutNote.js.map