...
const WhatsappTemplateTable = ({ data, searchQuery, onSearchChange, rowsPerPage, onRowsPerPageChange, totalRecords, dropdownOpen, onToggleDropdown, onEdit, onDelete }) => {
  return (
          ...
          <tbody>
            {data.length === 0 ? (
              ...
              data.map((item, index) => (
                <tr key={item.id}>
                  <td>{index + 1}</td>
                  <td>{item.templateName || item.name || '-'}</td>
                  <td>{truncate(item.message || item.content, MESSAGE_TRUNCATE_LENGTH)}</td>
                  <td>
