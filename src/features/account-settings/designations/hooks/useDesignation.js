...
export function useDesignation() {
    ...
    const applyFieldErrors = useCallback((errors, message, field, setFieldError) => {
        ...
        if (message) {
            ...
            if (lower.includes('designation')) {
                setFieldError('designationName', message);
    ...
    const handleAddDesignation = useCallback(async (values, { setSubmitting, resetForm, setFieldError }) => {
        pagination.setError('');
    ...
    const handleUpdateDesignation = useCallback(async (id, values, { setSubmitting, setFieldError }) => {
        pagination.setError('');
        pagination.setIsLoading(true);
        try {
            const requestData = { ...values, description: values.description.trim() };
...
//# sourceMappingURL=useDesignation.js.map