...
export function useEmailTemplate() {
    ...
    const pagination = useTableData({ fetchFn });
    ...
    const handleAddEmailTemplate = useCallback(async (values, { setSubmitting, resetForm, setFieldError }) => {
        pagination.setError('');
    ...
    const handleUpdateEmailTemplate = useCallback(async (id, values, { setSubmitting, setFieldError }) => {
        pagination.setError('');
        pagination.setIsLoading(true);
        try {
            const { templateName, subject, content, status } = values;
...
//# sourceMappingURL=useEmailTemplate.js.map